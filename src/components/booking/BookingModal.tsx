...
export function BookingModal({ isOpen, onClose, venue }: BookingModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 md:p-6 overflow-y-auto animate-fade-in">
      ...
      {/* Modal Container */}
      <div className="relative w-full max-w-xl bg-slate-950 border border-purple-500/30 rounded-2xl shadow-2xl z-10 overflow-hidden my-auto">
        
        {/* Modal Header */}
        <div className="p-4 sm:p-5 border-b border-white/10 bg-gradient-to-r from-purple-950/90 via-slate-950 to-indigo-950/90 flex items-center justify-between">
        ...
        <div className="px-5 py-3 border-t border-white/10 bg-slate-900 flex items-center justify-between text-xs text-slate-300 font-medium">
          <span className="flex items-center gap-1.5 font-bold text-emerald-400">
            <CheckCircle2 className="w-4 h-4 text-emerald-400" /> 100% Refundable Deposit Policy
          </span>
          <span className="text-slate-300 font-semibold flex items-center gap-1">
            <Lock className="w-3.5 h-3.5 text-purple-400" /> Secure Checkout
          </span>
        </div>

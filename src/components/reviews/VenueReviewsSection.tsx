      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div className="space-y-2">
          <span className="text-amber-400 text-xs font-bold uppercase tracking-wider flex items-center gap-1.5">
            <Star className="w-4 h-4 fill-amber-400 text-amber-400" /> Real Guest Feedback · 180+ Events Hosted
          </span>
        ...
        <button
          ...
          className="btn-ghost border border-white/10 text-slate-200 text-sm px-5 py-3 flex items-center gap-2 hover:border-purple-500 hover:text-white cursor-pointer shrink-0 active:scale-95 touch-manipulation font-semibold"
        >
          <MessageSquare className="w-4 h-4 text-purple-400" /> Write a Review
        </button>
      </div>

      {/* Ratings Overview Card */}
      <div className="glass-card p-6 sm:p-8 rounded-2xl border border-white/10 bg-slate-900/80 grid grid-cols-1 md:grid-cols-12 gap-6 items-center shadow-xl">
        {/* Left Rating Number */}
        <div className="md:col-span-4 text-center md:text-left border-b md:border-b-0 md:border-r border-white/10 pb-6 md:pb-0 md:pr-6 space-y-1">
          <div className="flex items-center justify-center md:justify-start gap-2">
            <span className="text-5xl font-black text-white font-heading">4.94</span>
            <div className="space-y-0.5 text-left">
              ...
              <p className="text-xs text-slate-400 font-medium">189 Verified Party Bookings</p>
            </div>
          </div>
          <p className="text-xs text-emerald-400 font-semibold flex items-center justify-center md:justify-start gap-1 pt-2">
            <ShieldCheck className="w-4 h-4 text-emerald-400" /> 100% Verified Guest Stays
          </p>
        </div>
        ...
        <div className="md:col-span-8 grid grid-cols-2 sm:grid-cols-4 gap-4 text-center">
          <div className="p-3 rounded-xl bg-slate-950/80 border border-white/10">
            <p className="text-[11px] text-slate-400 font-semibold">Sound & Karaoke</p>
            <p className="text-lg font-extrabold text-white mt-0.5 font-heading">4.98</p>
            <p className="text-[10px] text-emerald-400 font-bold">Top Rated</p>
          </div>
          <div className="p-3 rounded-xl bg-slate-950/80 border border-white/10">
            <p className="text-[11px] text-slate-400 font-semibold">Cleanliness</p>
            <p className="text-lg font-extrabold text-white mt-0.5 font-heading">4.96</p>
            <p className="text-[10px] text-slate-400 font-medium">Sanitized</p>
          </div>
          <div className="p-3 rounded-xl bg-slate-950/80 border border-white/10">
            <p className="text-[11px] text-slate-400 font-semibold">Lighting & Decor</p>
            <p className="text-lg font-extrabold text-white mt-0.5 font-heading">4.95</p>
            <p className="text-[10px] text-slate-400 font-medium">Neon Vibe</p>
          </div>
          <div className="p-3 rounded-xl bg-slate-950/80 border border-white/10">
            <p className="text-[11px] text-slate-400 font-semibold">Deposit Refund</p>
            <p className="text-lg font-extrabold text-emerald-400 mt-0.5 font-heading">100%</p>
            <p className="text-[10px] text-emerald-400 font-bold">&lt; 12 Hours</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {filteredReviews.map((rev) => {
          ...
          return (
            <div
              key={rev.id}
              className="glass-card p-6 rounded-2xl border border-white/10 bg-slate-900/70 space-y-4 flex flex-col justify-between shadow-lg"
            >
              <div className="space-y-3">
                {/* Header: User & Rating */}
                <div className="flex items-start justify-between gap-3">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-full bg-purple-950/80 border border-purple-500/40 text-purple-300 font-extrabold text-xs flex items-center justify-center shadow-sm">
                      {rev.avatarText}
                    </div>
                    <div>
                      <div className="flex items-center gap-1.5">
                        ...
                        {rev.verified && (
                          <span className="badge bg-emerald-500/20 text-emerald-300 border border-emerald-500/40 text-[9px] px-1.5 py-0.2 font-bold">
                            Verified Guest
                          </span>
                        )}
                      </div>
                      <p className="text-[11px] text-slate-400 font-medium">{rev.occasion}</p>
                    </div>
                  </div>
                ...
                {rev.hostReply && (
                  <div className="p-3 rounded-xl bg-purple-950/50 border-l-2 border-purple-500 text-xs space-y-1">
                    <p className="text-[10px] font-bold text-purple-300">Host Response (Rent-A-Vibe Team):</p>
                    <p className="text-[11px] text-slate-300 font-medium">{rev.hostReply}</p>
                  </div>
                )}
              </div>

              {/* Footer */}
              <div className="border-t border-white/10 pt-3 flex items-center justify-between text-[11px] text-slate-400 font-medium">
                <span>{rev.date}</span>
                <button
                  type="button"
        ...
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-4 animate-fade-in">
          <div className="glass-card max-w-lg w-full p-6 sm:p-7 border border-purple-500/30 shadow-2xl relative space-y-5 bg-slate-950 text-white rounded-2xl">
            ...
            <div className="space-y-1">
              <span className="badge bg-amber-500/20 text-amber-300 border border-amber-500/30 text-xs px-2.5 py-0.5 font-bold">
                Verified Feedback
              </span>
              <h3 className="text-xl font-extrabold text-white font-heading">Share Your Party Experience</h3>
            ...
            {submitted ? (
              <div className="p-8 text-center space-y-2">
                ...
                <p className="text-xs text-slate-300 font-medium">Your feedback has been published.</p>
              </div>
            ) : (
              <form onSubmit={handleSubmitReview} className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-xs font-bold text-slate-200 mb-1">Star Rating</label>
                  ...
                  <div>
                    <label className="block text-xs font-bold text-slate-200 mb-1">Occasion</label>
                    <select
                      value={occasion}
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-200 mb-1">Your Review</label>
                  <textarea
                    rows={3}
